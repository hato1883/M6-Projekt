...
class NormalNarrator(NarrationInterface):
    ...
    def generate_move_text(cls, origin: Position,
        ...
        if attacked_piece is None:
            ...
            return f"The {str(moved_piece.get_color()).lower()} {str(moved_piece.get_type()).lower()} on {cls.index_to_alpha_numeral(origin)} takes {str(attacked_piece.get_color()).lower()} {str(attacked_piece.get_type()).lower()} at {cls.index_to_alpha_numeral(destination)}."  # noqa E501
